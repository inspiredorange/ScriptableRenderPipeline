Shader "Hidden/HDRP/DebugFullScreen"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2DX(_DebugFullScreenTexture);
            ...
            {
                float2 velocityNDC;
                DecodeVelocity(SAMPLE_TEXTURE2DX(_DebugFullScreenTexture, s_point_clamp_sampler, coords), velocityNDC);
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                {
                    float4 color = SAMPLE_TEXTURE2DX(_DebugFullScreenTexture, s_point_clamp_sampler, input.texcoord);
                    return color;
                }
                // SSAO
                if (_FullScreenDebugMode == FULLSCREENDEBUGMODE_SSAO)
                {
                    return 1.0f - SAMPLE_TEXTURE2DX(_DebugFullScreenTexture, s_point_clamp_sampler, input.texcoord).xxxx;
                }
                ...
                if (_FullScreenDebugMode == FULLSCREENDEBUGMODE_DEPTH_PYRAMID)
                {
                    ...
                    float depth = LOAD_TEXTURE2DX(_DepthPyramidTexture, pixCoord + mipOffset).r;

...
{
    float4 src;
    src.xyz = mul(decalToWorld, UnpackNormalmapRGorAG(SAMPLE_TEXTURE2DX_LOD(_DecalAtlas2D, _trilinear_clamp_sampler_DecalAtlas2D, texCoords, lod))) * 0.5f + 0.5f;
    src.w = blend;
...
void ApplyBlendDiffuse(inout float4 dst, inout int matMask, float2 texCoords, float4 src, int mapMask, inout float blend, float lod, int diffuseTextureBound)
{
    ...
    {
        src *= SAMPLE_TEXTURE2DX_LOD(_DecalAtlas2D, _trilinear_clamp_sampler_DecalAtlas2D, texCoords, lod);
    }
    src.w *= blend;
...
{
    float4 src = SAMPLE_TEXTURE2DX_LOD(_DecalAtlas2D, _trilinear_clamp_sampler_DecalAtlas2D, texCoords, lod);
...
DecalSurfaceData GetDecalSurfaceData(PositionInputs posInput, inout float alpha)
{
    ...
    mask = UnpackByte(LOAD_TEXTURE2DX(_DecalHTileTexture, posInput.positionSS / 8).r);
#endif

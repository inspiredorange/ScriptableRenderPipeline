...
TEXTURE2DX(_GBufferTexture0);
TEXTURE2DX(_GBufferTexture1);
TEXTURE2DX(_GBufferTexture2);
TEXTURE2DX(_GBufferTexture3); // Bake lighting and/or emissive
TEXTURE2DX(_GBufferTexture4); // Light layer or shadow mask
TEXTURE2DX(_GBufferTexture5); // shadow mask

TEXTURE2DX(_LightLayersTexture);
#ifdef SHADOWS_SHADOWMASK
TEXTURE2DX(_ShadowMaskTexture); // Alias for shadow mask, so we don't need to know which gbuffer is used for shadow mask
#endif
...
uint DecodeFromGBuffer(uint2 positionSS, uint tileFeatureFlags, out BSDFData bsdfData, out BuiltinData builtinData)
{
    ...
    GBufferType0 inGBuffer0 = LOAD_TEXTURE2DX(_GBufferTexture0, positionSS);
    GBufferType1 inGBuffer1 = LOAD_TEXTURE2DX(_GBufferTexture1, positionSS);
    GBufferType2 inGBuffer2 = LOAD_TEXTURE2DX(_GBufferTexture2, positionSS);

    // BuiltinData
    builtinData.bakeDiffuseLighting = LOAD_TEXTURE2DX(_GBufferTexture3, positionSS).rgb;  // This also contain emissive (and * AO if no lightlayers)
    ...
    {
        float4 inGBuffer4 = LOAD_TEXTURE2DX(_LightLayersTexture, positionSS);        
...
#ifdef SHADOWS_SHADOWMASK
    float4 shadowMaskGbuffer = LOAD_TEXTURE2DX(_ShadowMaskTexture, positionSS);
                                                    ...
                                                    inout float    reflectionHierarchyWeight)
{
    ...
    float4 ssrLighting = LOAD_TEXTURE2DX(_SsrLightingTexture, posInput.positionSS);
                                                    ...
                                                    inout float hierarchyWeight)
{
    ...
    float hitDeviceDepth = LOAD_TEXTURE2DX_LOD(_DepthPyramidTexture, TexCoordStereoOffset(hit.positionSS), 0).r;
    ...
    float3 preLD = SAMPLE_TEXTURE2DX_LOD(_ColorPyramidTexture, s_trilinear_clamp_sampler,

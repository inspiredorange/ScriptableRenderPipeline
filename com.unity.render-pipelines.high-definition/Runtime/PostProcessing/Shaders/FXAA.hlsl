...
float3 Fetch(TEXTURE2DX_ARGS(_InputTexture, _InputTextureSampler), float2 coords, float2 offset)
{
    float2 uv = saturate(coords + offset) * _ScreenToTargetScale.xy;
    return SAMPLE_TEXTURE2DX_LOD(_InputTexture, _InputTextureSampler, uv, 0.0).xyz;
}

float3 Load(TEXTURE2DX(_InputTexture), int2 icoords, int idx, int idy)
{
...
void RunFXAA(TEXTURE2DX_ARGS(_InputTexture, _InputTextureSampler), inout float3 outColor, uint2 positionSS, float2 positionNDC)
{
    {
        // Edge detection
        ...
        // Blur
        float3 rgb03 = Fetch(TEXTURE2DX_PARAM(_InputTexture, _InputTextureSampler), positionNDC, dir * (0.0 / 3.0 - 0.5));
        float3 rgb13 = Fetch(TEXTURE2DX_PARAM(_InputTexture, _InputTextureSampler), positionNDC, dir * (1.0 / 3.0 - 0.5));
        float3 rgb23 = Fetch(TEXTURE2DX_PARAM(_InputTexture, _InputTextureSampler), positionNDC, dir * (2.0 / 3.0 - 0.5));
        float3 rgb33 = Fetch(TEXTURE2DX_PARAM(_InputTexture, _InputTextureSampler), positionNDC, dir * (3.0 / 3.0 - 0.5));

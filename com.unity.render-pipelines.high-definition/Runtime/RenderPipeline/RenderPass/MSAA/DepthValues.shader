Shader "Hidden/HDRP/DepthValues"
{
    HLSLINCLUDE
        ...
        TEXTURE2DX_MSAA(float, _DepthTextureMS);
        TEXTURE2DX_MSAA(float4, _NormalTextureMS);
        ...
        FragOut Frag1X(Varyings input)
        {
            ...
            float depthVal = LOAD_TEXTURE2DX_MSAA(_DepthTextureMS, pixelCoords, 0).x;
            fragO.depthValues = float4(depthVal, depthVal, depthVal, 0.0f);
            fragO.normal = LOAD_TEXTURE2DX_MSAA(_NormalTextureMS, pixelCoords, 0);
        ...
        FragOut Frag2X(Varyings input)
        {
            ...
            {
                float depthVal = LOAD_TEXTURE2DX_MSAA(_DepthTextureMS, pixelCoords, sampleIdx).x;
